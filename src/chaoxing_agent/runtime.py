...
class ActionRuntime:
    ...
    @staticmethod
    def _observable_postcondition(risk: str, result: Any) -> dict[str, Any]:
        if isinstance(result, dict):
            reported = result.get("postcondition")
        ...
        return {
            "observed": False,
            "kind": "verification_not_reported",
            "summary": "动作返回成功，但结果中没有可观察后置条件，不能据此声称已完成。",
        }
    ...
    async def execute(
        ...
        if spec.risk.requires_confirmation:
            ...
            if not spec.live_verified:
                summary = f"{summary}；此功能尚未完成真实账号验收，可能无法成功"
        ...
        except ChaoxingLoginVerificationRequired as exc:
            return {
        ...
        return {
            ...
            "live_verified": spec.live_verified,
            "result": result,
            "postcondition": self._observable_postcondition(spec.risk.value, result),
        }
